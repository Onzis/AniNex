...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const turboPlayer = kinoboxData.data.find((player: any) => player.type === "Turbo");
    
    if (!turboPlayer || !turboPlayer.iframeUrl) {
      return NextResponse.json({ error: 'Turbo player not found' }, { status: 404 });
    }

    // Добавляем параметр эпизода, если он есть
    let playerUrl = turboPlayer.iframeUrl;
    ...
    console.error('Error fetching Turbo player:', error);
    return NextResponse.json(
      { error: 'Failed to fetch Turbo player' },
      { status: 500 }
    );
  }
}